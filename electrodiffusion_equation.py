...
class ElectroDiffusionEquation():
    ...
    @classmethod
    def static_1D_real_anizotropic_mod(self, cnf, properties, bound_conditions):
        # cnf = {
        #     'N': 4 * 10 ** 2,
        #     'dn': 10 ** (-10),
        #     'eps0': 8.85 * 10 ** (-12)
        ...
        diffusion_left = np.concatenate((0.5 * (diffusion[:-2] + diffusion[1:-1]),np.zeros(2)))
        diffusion_right = np.concatenate((np.zeros(2),0.5 * (diffusion[1:-1] + diffusion[2:])))
        diffusion_center = np.concatenate((np.zeros(1),diffusion[:-2] + diffusion[2:],np.zeros(1)))
        ...
        matrix_potential_equations = sp.dia_matrix((
            ...
            shape=(N - 2, 2 * N))
        matrix_charge_equations = sp.dia_matrix((
            [np.concatenate((zeros_N, diffusion_left)),
             - np.concatenate((zeros_N, diffusion_center + sigma * inv_eps_dn2)),
             np.concatenate((zeros_N, diffusion_right))],
            [N, N + 1, N + 2]),
            shape=(N - 2, 2 * N))
        ...
        matrix_charge_start = sp.coo_matrix((
            [-sigma[0] / diffusion[0] * charge_start[1],
             sigma[0] / diffusion[0] * charge_start[1],
             -charge_start[1] + dn * charge_start[0] / diffusion[0],
             charge_start[1]],
            ([0, 0, 0, 0], [0, 1, N, N + 1])), shape=(1, 2 * N))
        matrix_charge_end = sp.coo_matrix((
            [-sigma[-1] / diffusion[-1] * charge_end[1],
             sigma[-1] / diffusion[-1] * charge_end[1],
             -charge_end[1] + dn * charge_end[0] / diffusion[-1],
             charge_end[1]],
            ([0, 0, 0, 0], [N - 2, N - 1, 2 * N - 2, 2 * N - 1])), shape=(1, 2 * N))
